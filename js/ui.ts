import type { Produto, Depoimento } from "./api.ts";

function escAttr(str: string | number): string {
  return String(str)
    .replace(/&/g, "&amp;")
    .replace(/"/g, "&quot;")
    .replace(/</g, "&lt;")
...
export function renderizarProdutos(produtos: Produto[]): void {
...
export function renderizarDepoimentos(depoimentos: Depoimento[]): void {
...
export function atualizarContadorCarrinho(): void {
  ...
  if (!counter) return;
  const carrinho: { quantidade: number }[] =
